<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-900 leading-tight">
            {{ __('Edit Virtual Event') }}
        </h2>
    </x-slot>

    <div class="max-w-4xl">
        <x-ui.card>
            <form method="POST" action="{{ route('employer.virtual-events.update', $event) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="5" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus-visible:ring-2 focus-visible:ring-offset-0">{{ old('description', $event->description) }}</textarea>
                    ...
                    <div>
                        ...
                        <select id="platform" name="platform" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus-visible:ring-2 focus-visible:ring-offset-0" required>
                            ...
                            @foreach ($platforms as $key => $label)
                                <option value="{{ $key }}" {{ old('platform', $event->platform) === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        ...
                        <x-text-input id="start_time" name="start_time" type="time" class="mt-1 block w-full" :value="old('start_time', $event->start_time)" required />
                        <x-input-error :messages="$errors->get('start_time')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="end_time" :value="__('End Time')" />
                        <x-text-input id="end_time" name="end_time" type="time" class="mt-1 block w-full" :value="old('end_time', $event->end_time)" required />
                ...
                <div>
                    ...
                    <x-text-input id="meeting_link" name="meeting_link" type="url" class="mt-1 block w-full" :value="old('meeting_link', $event->meeting_link)" required />
                ...
                <div>
                    ...
                    <x-text-input id="registration_deadline" name="registration_deadline" type="datetime-local" class="mt-1 block w-full" :value="old('registration_deadline', $event->registration_deadline ? $event->registration_deadline->format('Y-m-d\TH:i') : '')" />
                    <x-input-error :messages="$errors->get('registration_deadline')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="status" :value="__('Status')" />
                    <select id="status" name="status" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus-visible:ring-2 focus-visible:ring-offset-0" required>
