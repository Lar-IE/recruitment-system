                            ...
                            <table class="min-w-full text-sm">
                                <thead class="text-left text-gray-500">
                                    <tr>
                                        ...
                                        <th class="py-2 pr-4">{{ __('Current Status') }}</th>
                                        <th class="py-2 pr-4">{{ __('Applied At') }}</th>
                                        <th class="py-2 pr-4 text-right">{{ __('Update Status') }}</th>
                                    </tr>
                                </thead>
                                <tbody class="text-gray-700">
                                    @foreach ($applications as $application)
                                        <tr class="border-t align-top">
                                            ...
                                            <td class="py-3 pr-4">
                                                <form method="POST" action="{{ route('employer.ats.status', $application) }}" class="space-y-2 text-right">
                                                    @csrf
                                                    <select name="status" class="block w-full text-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                                        @foreach ($statuses as $key => $label)
                                                            <option value="{{ $key }}" {{ $application->current_status === $key ? 'selected' : '' }}>{{ $label }}</option>
                                                        @endforeach
                                                    </select>
                                                    <textarea name="note" rows="2" class="block w-full text-xs border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" placeholder="{{ __('Optional note') }}"></textarea>
