                        ...
                        @foreach ($types as $key => $label)
                            ...
                            <div class="border rounded-lg p-4 space-y-3">
                                ...
                                @if ($document)
                                    ...
                                    @if (!$isHired)
                                        <form method="POST" action="{{ route('jobseeker.documents.destroy', $document) }}" onsubmit="return confirm('{{ __('Are you sure you want to remove this document? This action cannot be undone.') }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="w-full mt-1 inline-flex items-center justify-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 focus:bg-red-500 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition">
                                                {{ __('Remove') }}
                                            </button>
                                        </form>
                                    @endif
